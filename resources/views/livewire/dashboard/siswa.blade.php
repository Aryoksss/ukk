        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center mb-6">
                    <h2 class="text-2xl font-semibold text-gray-800 mb-3 sm:mb-0">Daftar Siswa</h2>
                    <div class="flex w-full sm:w-auto">
                        <div class="flex items-center w-full sm:w-auto">
                            <div class="relative flex-grow sm:flex-grow-0">
                                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari siswa..." 
                ...
                @if(((is_object($siswa) && $siswa->isEmpty()) || (is_array($siswa) && empty($siswa))) && $search)
                <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700">Tidak ada siswa yang ditemukan dengan kata kunci "{{ $search }}"</p>
                        </div>
                    </div>
                </div>
                @endif
                    ...
                    <ul role="list" class="divide-y divide-gray-200">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-12 w-12 bg-indigo-100 rounded-full flex items-center justify-center">
                                            <span class="text-base font-semibold text-indigo-600">{{ strtoupper(substr($s->nama, 0, 2)) }}</span>
                                        </div>
                                        <div class="ml-4">
                                    ...
                                    <div class="flex flex-col sm:flex-row sm:space-x-2">
                                        <button wire:click="showDetailModal({{ $s->id }})" 
                                            class="px-3 py-1.5 bg-indigo-500 text-white text-xs font-semibold rounded-md hover:bg-indigo-600 transition duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-75 shadow-sm">
                                            Detail
                                        </button>
                                    </div>
                                </div>
                        ...
                        @empty
                            <li class="px-4 py-12 sm:px-6 text-center text-gray-500">
                                ...
                                <p class="mt-1 text-sm text-gray-500">Belum ada data siswa yang tersedia.</p>
                            </li>
                        @endforelse
                    </ul>
                                        ...
                                        <div class="mt-4 border-t border-gray-200 pt-4">
                                            <dl class="space-y-4">
                                                <div>
                                                    ...
                                                    <dd class="mt-1 text-sm text-gray-900">{{ $selectedSiswa->nis }}</dd>
                                                </div>
                                                <div>
                                                ...
                                                <div>
                                                    ...
                                                    <dd class="mt-1 text-sm text-gray-900">{{ $selectedSiswa->alamat ?? 'Belum ada data' }}</dd>
                                                </div>
                                                <div>
                                                    <dt class="text-sm font-medium text-gray-500">Tempat PKL</dt>
                                                    <dd class="mt-1 text-sm text-gray-900">
                                                        @if(is_object($selectedSiswa->pkl) && !$selectedSiswa->pkl instanceof \Illuminate\Support\Collection)
                                                            {{ $selectedSiswa->pkl->industri->nama ?? 'Belum PKL' }}
                                                        @elseif($selectedSiswa->pkl instanceof \Illuminate\Support\Collection && $selectedSiswa->pkl->isNotEmpty())
                                                            {{ $selectedSiswa->pkl->first()->industri->nama ?? 'Belum PKL' }}
                                                        @else
                                                            Belum PKL
                                                        @endif
                                                    </dd>
                                                ...
                                                <div>
                                                    <dt class="text-sm font-medium text-gray-500">Tanggal Mulai PKL</dt>
                                                    <dd class="mt-1 text-sm text-gray-900">
                                                        @if(is_object($selectedSiswa->pkl) && !$selectedSiswa->pkl instanceof \Illuminate\Support\Collection)
                                                            {{ isset($selectedSiswa->pkl->mulai) && !empty($selectedSiswa->pkl->mulai) ? \Carbon\Carbon::parse($selectedSiswa->pkl->mulai)->locale('id')->isoFormat('D MMMM Y') : '-' }}
                                                        @elseif($selectedSiswa->pkl instanceof \Illuminate\Support\Collection && $selectedSiswa->pkl->isNotEmpty())
                                                            {{ isset($selectedSiswa->pkl->first()->mulai) && !empty($selectedSiswa->pkl->first()->mulai) ? \Carbon\Carbon::parse($selectedSiswa->pkl->first()->mulai)->locale('id')->isoFormat('D MMMM Y') : '-' }}
                                                        @else
                                                            -
                                                        @endif
                                                    </dd>
                                                ...
                                                <div>
                                                    <dt class="text-sm font-medium text-gray-500">Tanggal Selesai PKL</dt>
                                                    <dd class="mt-1 text-sm text-gray-900">
                                                        @if(is_object($selectedSiswa->pkl) && !$selectedSiswa->pkl instanceof \Illuminate\Support\Collection)
                                                            {{ isset($selectedSiswa->pkl->selesai) && !empty($selectedSiswa->pkl->selesai) ? \Carbon\Carbon::parse($selectedSiswa->pkl->selesai)->locale('id')->isoFormat('D MMMM Y') : '-' }}
                                                        @elseif($selectedSiswa->pkl instanceof \Illuminate\Support\Collection && $selectedSiswa->pkl->isNotEmpty())
                                                            {{ isset($selectedSiswa->pkl->first()->selesai) && !empty($selectedSiswa->pkl->first()->selesai) ? \Carbon\Carbon::parse($selectedSiswa->pkl->first()->selesai)->locale('id')->isoFormat('D MMMM Y') : '-' }}
                                                        @else
                                                            -
                                                        @endif
                                                    </dd>
