<nav x-data="{ open: false }" class="bg-gradient-to-r from-indigo-600 to-blue-500 shadow-md">
            ...
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center">
                        <div class="bg-white p-1.5 rounded-lg shadow-md transform transition-transform duration-300 hover:scale-105">
                            <x-application-mark class="block h-7 w-auto" />
                        </div>
                        <span class="ml-2 text-white font-bold text-lg hidden md:block">SI-PKL</span>
                    </a>
                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gradient-to-b from-indigo-800 to-indigo-900 shadow-lg transform transition-all duration-300 ease-in-out">
        <!-- Welcome Banner -->
        <div class="px-4 pt-4 pb-3 bg-indigo-900/50 mb-2">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <p class="text-xs text-indigo-200">Selamat Datang</p>
                        <h4 class="font-bold text-white">PKL Management System</h4>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="flex items-center">
                    @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                        <div class="shrink-0 me-3">
                            <img class="size-10 rounded-full object-cover border-2 border-indigo-400" src="{{ Auth::user()->profile_photo_url }}" alt="{{ Auth::user()->name }}" />
                        </div>
                    @endif

                    <div>
                        <div class="font-medium text-base text-white">{{ Auth::user()->name }}</div>
                        <div class="font-medium text-xs text-indigo-200">{{ Auth::user()->email }}</div>
                    </div>
                </div>
            </div>

            <div class="mt-3 space-y-1 border-t border-indigo-700 pt-2">
                <p class="px-4 py-1 text-xs text-indigo-300 font-semibold uppercase tracking-wider">Pengaturan Akun</p>
                ...
                <form method="POST" action="{{ route('logout') }}" x-data>
                    ...
                    <x-responsive-nav-link href="{{ route('logout') }}" class="text-white hover:bg-red-600"
                                   @click.prevent="$root.submit();">
                        <div class="flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
